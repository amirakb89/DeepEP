...
__host__ __device__ __forceinline__ int64_t get_num_bytes_per_rdma_token(int hidden_int4,
                                                                         int num_scales,
                                                                         int num_topk_idx,
...
__global__ void __launch_bounds__(((kNumDispatchRDMASenderWarps + 1 + NUM_MAX_NVL_PEERS) * kWarpSize), 1)
    ...
    // NOTES: `rdma_send_channel_window` means the ongoing kWarpSize transactions' status
    __shared__ int rdma_send_channel_lock[kNumRDMARanks];
    __shared__ int rdma_send_channel_tail[kNumRDMARanks];
    __shared__ uint32_t rdma_send_channel_window[kNumRDMARanks];
    auto sync_rdma_sender_smem = []() { asm volatile("barrier.sync 0, %0;" ::"r"((kNumDispatchRDMASenderWarps + 1) * kWarpSize)); };
    ...
    auto sync_forwarder_smem = []() { asm volatile("barrier.sync 1, %0;" ::"r"((NUM_MAX_NVL_PEERS + 1) * kWarpSize)); };
    ...
    if (warp_role == WarpRole::kRDMASender) {
        ...
        for (token_idx = token_start_idx; token_idx < token_end_idx; ++token_idx) {
            ...
            if (is_token_in_rank_uint64 != 0) {
                ...
                if (offset == 0) {
                    auto num_empty_slots = (~window) == 0 ? kWarpSize : __ffs(~window) - 1;
...
__device__ int combine_token(bool is_token_in_rank,
    ...
    if constexpr (kUseTMA) {
        ...
        EP_DEVICE_ASSERT(hidden_int4 % kWarpSize == 0);
        ...
        for (int shifted = 0, iter = 0; shifted < hidden_int4; shifted += kWarpSize, iter += 1) {
            const int stage_idx = iter % kNumStages;
            const int next_stage_idx = (iter + 1) % kNumStages;

            // Prefetch next stage
            if (shifted + kWarpSize < hidden_int4) {
                if (lane_id < num_topk_ranks)
                    tma_load_1d(tma_load_buffer(next_stage_idx, lane_id),
                                get_addr_fn(topk_ranks[lane_id], slot_indices[lane_id], shifted + kWarpSize),
...
__global__ void __launch_bounds__((kNumForwarders + 1) * kWarpSize, 1) combine(int4* combined_x,
                                                                        float* combined_topk_weights,
    ...
    if (warp_role == WarpRole::kNVLSender) {
        ...
        auto sync_forwarder_smem = [=]() { asm volatile("barrier.sync 0, %0;" ::"r"((kNumForwarders + 1) * kWarpSize)); };
        auto sync_rdma_receiver_smem = [=]() { asm volatile("barrier.sync 1, %0;" ::"r"((kNumRDMAReceivers + 1) * kWarpSize)); };
        ...
        if (warp_role == WarpRole::kNVLAndRDMAForwarder) {
            ...
            constexpr int kNumTMALoadBytes = sizeof(int4) * kWarpSize;
            constexpr int kNumTMABufferBytesPerStage = kNumTMALoadBytes * (NUM_MAX_NVL_PEERS + 1) + 16;
            ...
            if (lane_id < kNumStages) {
                mbarrier_init(tma_mbarrier(lane_id), kWarpSize);
                fence_barrier_init();
            }
            __syncwarp();
...
#define COMBINE_LAUNCH_CASE(num_rdma_ranks)                                           \
    ...
    SETUP_LAUNCH_CONFIG(num_channels * 2, (num_forwarder_warps + 1) * kWarpSize, stream);
