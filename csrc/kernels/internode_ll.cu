...
dispatch(void* packed_recv_x, float* packed_recv_x_scales,
        ...
        for (int token_idx = sm_id; token_idx < num_tokens; token_idx += num_sms) {
            ...
            if (dst_expert_idx >= 0) {
                ...
                if (dst_rank != rank) {
                    ...
                    UNROLLED_WARP_COPY(2, lane_id, num_int4_per_msg, dst_int4_ptr, src_int4_ptr, ld_nc_global, st_na_global);
                }
    ...
    if (responsible_expert_idx < num_experts and sub_warp_id == 0 and lane_id == 0) {
        ...
        if (dst_rank != rank) {
           ...
           nvshmemi_ibgda_amo_nonfetch_add(rdma_recv_count + dst_expert_local_idx * num_ranks + rank, -num_tokens_sent - 1, dst_rank, dst_expert_local_idx);
            ...
            st_na_relaxed(reinterpret_cast<int64_t *>(rdma_recv_count + dst_expert_local_idx * num_ranks + rank), -num_tokens_sent - 1);
        }
    ...
    if (responsible_expert_idx < num_experts) {
        ...
        if (sub_warp_id == 1 and lane_id == 0) {
            while ((num_recv_tokens = ld_volatile_global(reinterpret_cast<int*>(rdma_recv_count + local_expert_idx * num_ranks + src_rank))) == 0);
        ...
        for (int i = sub_warp_id; i < num_recv_tokens; i += kNumWarpsPerGroup) {
            ...
            UNROLLED_WARP_COPY(2, lane_id, hidden_int4, dst_data, src_data, ld_nc_global, st_na_global);
...
combine(void* combined_x,
    ...
    if (responsible_expert_idx < num_experts) {
        ...
        for (int token_idx = offset + sub_warp_id; token_idx < offset + num_tokens_to_send; token_idx += kNumWarpsPerGroup) {
            ...
            if (dst_rank == rank) {
                const auto dst_int4_ptr = reinterpret_cast<int4*>(dst_ptr);
                UNROLLED_WARP_COPY(2, lane_id, hidden_bf16_int4, dst_int4_ptr, x_int4, ld_nc_global, st_na_global);
            } else {
                const auto buf_int4_ptr = reinterpret_cast<int4*>(buf_ptr);
                if (not zero_copy)
                    UNROLLED_WARP_COPY(2, lane_id, hidden_bf16_int4, buf_int4_ptr, x_int4, ld_nc_global, st_na_global);
    ...
    if (responsible_expert_idx < num_experts) {
        ...
        if (sub_warp_id == 0 and lane_id == 0){
            while (ld_volatile_global(reinterpret_cast<int*>(rdma_recv_flag + responsible_expert_idx)) == 0);
        }
    }
    grid_barrier(global_atomic_counter, num_sms);
